var originalFlavors = [
  ...
  // "Guava" //This tests if the function works
];
...
function addFlavor(newFlavor, flavorsList) {
  flavorsList.unshift(newFlavor);
...
function removeLastFlavor(flavorsList) {
  flavorsList.pop();
...
function removeFlavorByName(flavorList, flavorName) {
  flavorList.splice(flavorList.indexOf(flavorName), 1);
...
function copy(flavorsCopy, array) {
...
copy(newFlavorsList, originalFlavors);
...
function filterByWord(string, array) {
  let chocolateList = [];

  for (let i = 0; i < array.length; i++) {
    if (array[i].includes(string)) {
      //find chocolate inside each item of the array
